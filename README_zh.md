...
Agent Chrome MCP 是一个基于chrome插件的 **模型上下文协议 (MCP) 服务器**，它将您的 Chrome 浏览器功能暴露给 Claude 等 AI 助手，实现复杂的浏览器自动化、内容分析和语义搜索等。与传统的浏览器自动化工具（如playwright）不同，**Agent Chrome MCP server**直接使用您日常使用的chrome浏览器，基于现有的用户习惯和配置、登录态，让各种大模型或者各种chatbot都可以接管你的浏览器，真正成为你的日常助手
...
那么你的最终路径就是：/Users/xxx/Library/pnpm/global/5/node_modules/agent-chrome-mcp/dist/mcp/mcp-server-stdio.js
...
{
  "mcpServers": {
    "agent-chrome-mcp-stdio": {
      "command": "npx",
      "args": [
        "node",
        "/Users/xxx/Library/pnpm/global/5/node_modules/agent-chrome-mcp/dist/mcp/mcp-server-stdio.js"
      ]
    }
  }
}
